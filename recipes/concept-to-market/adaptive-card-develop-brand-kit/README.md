# Develop brand kit Status Adaptive Card

Produces a reusable Adaptive Card JSON snapshot of develop brand kit status for embedding in dashboards, emails, or Teams.
...
Gives any consuming app (Teams, Outlook, custom dashboard) a single canonical develop brand kit status card so different surfaces always show the same numbers.

## What it does

Generates an Adaptive Card JSON file with current develop brand kit KPIs and RAG indicators.
...
![Placeholder screenshot for Develop brand kit Status Adaptive Card](screenshots/01-placeholder.svg "Placeholder - replace with a real screenshot captured against your tenant.")
