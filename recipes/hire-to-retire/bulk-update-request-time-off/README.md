# Request time off Bulk Field Update

Applies a bulk field update across request time off records from an input list, with dry-run preview before commit.
...
Avoids the click-by-click drudgery (and risk of inconsistency) when request time off records need a coordinated change - and the dry-run preview catches mistakes before they hit the system.
...
![Placeholder screenshot for Request time off Bulk Field Update](screenshots/01-placeholder.svg "Placeholder - replace with a real screenshot captured against your tenant.")
