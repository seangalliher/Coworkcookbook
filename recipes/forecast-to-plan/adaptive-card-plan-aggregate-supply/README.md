# Plan aggregate supply Status Adaptive Card

Produces a reusable Adaptive Card JSON snapshot of plan aggregate supply status for embedding in dashboards, emails, or Teams.
...
Gives any consuming app (Teams, Outlook, custom dashboard) a single canonical plan aggregate supply status card so different surfaces always show the same numbers.

## What it does

Generates an Adaptive Card JSON file with current plan aggregate supply KPIs and RAG indicators.
...
![Placeholder screenshot for Plan aggregate supply Status Adaptive Card](screenshots/01-placeholder.svg "Placeholder - replace with a real screenshot captured against your tenant.")
