# Enter sales orders Status Adaptive Card

Produces a reusable Adaptive Card JSON snapshot of enter sales orders status for embedding in dashboards, emails, or Teams.
...
Gives any consuming app (Teams, Outlook, custom dashboard) a single canonical enter sales orders status card so different surfaces always show the same numbers.

## What it does

Generates an Adaptive Card JSON file with current enter sales orders KPIs and RAG indicators.
...
![Placeholder screenshot for Enter sales orders Status Adaptive Card](screenshots/01-placeholder.svg "Placeholder - replace with a real screenshot captured against your tenant.")
