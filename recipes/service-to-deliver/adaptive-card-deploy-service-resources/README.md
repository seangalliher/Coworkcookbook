# Deploy service resources Status Adaptive Card

Produces a reusable Adaptive Card JSON snapshot of deploy service resources status for embedding in dashboards, emails, or Teams.
...
Gives any consuming app (Teams, Outlook, custom dashboard) a single canonical deploy service resources status card so different surfaces always show the same numbers.

## What it does

Generates an Adaptive Card JSON file with current deploy service resources KPIs and RAG indicators.
...
![Placeholder screenshot for Deploy service resources Status Adaptive Card](screenshots/01-placeholder.svg "Placeholder - replace with a real screenshot captured against your tenant.")
