...
RECIPES: List[Recipe] = [
    ...
    Recipe(
        ...
        tenant_caveat=(
            "Validated against a live Cowork tenant on 2026-05-23. The agent correctly routed to the D365 ERP plugin, "
            "discovered the right entity (TrialBalanceFiscalYearSnapshots), and queried LedgerJournalLines / "
            "GeneralLedgerActivities. In the USMF demo dataset there were no posted GL transactions in the current "
            "or prior period, so the agent honestly refused to fabricate balances and offered three remediation paths "
            "instead. Run this recipe against a tenant with current-period GL postings to get a fully-populated workbook."
        ),
        prompt=(
